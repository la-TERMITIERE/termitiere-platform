...
export default function Table({
  ...
  return (
    <div className={`overflow-auto rounded-lg border border-gray-100 ${stickyHeader ? '' : ''}`}
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="bg-gray-50 text-left text-xs uppercase tracking-wide text-gray-500">
            {columns.map((c, i) => (
              <th key={c.key}
                className={`bg-gray-50 px-3 py-2.5 font-semibold ${alignCls(c.align)} ${stickyHeader ? 'sticky top-0' : ''} ${c.sticky ? 'sticky' : ''}`}
                style={{
          ...
          {rows.length === 0 ? (
            ...
            rows.map((row) => (
              ...
              >
                {columns.map((c, i) => (
                  <td key={c.key}
                    className={`px-3 py-2.5 ${alignCls(c.align)} ${c.sticky ? 'sticky bg-white group-hover:bg-gray-50' : ''}`}
